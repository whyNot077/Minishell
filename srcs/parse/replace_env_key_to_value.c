...
/*   Updated: 2023/04/24 14:23:23 by minkim3          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	replace_key_in_the_string(char **value, char *key, char *env_value, size_t *i)
{
	char	*prefix;
	char	*suffix;
	char	*tmp;
	...
	prefix = ft_substr(*value, 0, *i - 1);
	suffix = ft_substr(*value, *i + ft_strlen(key), ft_strlen(*value) - (*i + ft_strlen(key)));
...
void	replace_env_key_to_value(char **value, char **env)
{
	...
	while ((*value)[++i])
	{
		...
		if ((*value)[i] == '$' && in_single_quote == -1)
		{
			...
			env_value = get_env_value(key, env, &i);
